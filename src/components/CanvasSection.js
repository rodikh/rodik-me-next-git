...
export default class CanvasSection extends React.Component {
    ...
    demoTypes = {
        ...
        'particles': {
            up: (canvas, options) => {
                const exploder = new ParticleExploder(canvas, canvas, {gravity: 0.02, color: '255,0,0'});
                const emitter1 = new ParticleEmitter({x: 50, y: 50}, {x:3, y:0}, canvas, {color: '255,255,255', isDecaying: true});
        ...
        'quadtree': {
            up: (canvas, options) => {
                const engine = new ParticleEngine(canvas, {color: '255,255,255', particlesAmount: 50, maxLineDistance: 50, useTree: true, gridLines: true, nodeConnections: true});
                this.drawQueue.push(engine);
            },
            down: () => {}
        }
